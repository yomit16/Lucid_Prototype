...
export async function POST(request: NextRequest) {
  try {
    ...
    // helper: append a practice object into practice jsonb array
    const savePractice = async (uid: string | null, item: any) => {
      if (!uid) return
      try {
        // Prefer RPC if available
        try {
          const { error: rpcError } = await supabase.rpc('append_practice_item', { p_user: uid, p_item: item })
          if (!rpcError) return
          console.warn('[assistant] append_practice_item rpc error', { user: uid, rpcError })
        } catch (e) {
          // RPC not available or failed - fallback to reading/upserting the array
        }

        const { data: existing, error: selectErr } = await supabase.from('chatbot_user_interactions').select('practice').eq('user_id', uid).single()
        if (selectErr) {
          const { error: upsertErr } = await supabase.from('chatbot_user_interactions').upsert([{ user_id: uid, practice: [item], updated_at: new Date().toISOString() }], { onConflict: 'user_id' })
          if (upsertErr) console.warn('[assistant] savePractice initial upsert error', { user: uid, upsertErr })
          return
        }
        let arr: any[] = []
        if (existing && Array.isArray(existing.practice)) arr = existing.practice
        arr.push(item)
        const { data: upsertData, error: upsertErr } = await supabase.from('chatbot_user_interactions').upsert([{ user_id: uid, practice: arr, updated_at: new Date().toISOString() }], { onConflict: 'user_id' })
        if (upsertErr) console.warn('[assistant] savePractice upsert error', { user: uid, upsertErr })
        else try { console.log('[assistant] savePractice ok', { user: uid, rows: Array.isArray(upsertData) ? upsertData.length : null }) } catch(e) {}
      } catch (e) {
        console.warn('[assistant] savePractice failed', e)
      }
    }
